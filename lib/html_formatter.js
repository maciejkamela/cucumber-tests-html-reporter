...
module.exports = (function() {
    ...
    var template = grunt.file.read(templates.headerTemplate),
      header = grunt.template.process(template, {
        data: {
          ...
          scenariosSummary: scenariosNumber,
          passedScenarios: passedScenarios,
          failedScenarios: scenariosNumber - passedScenarios,
          stepsSummary: stepsNumber,
  ...
  function generateHTML(testResults) {
    ...
    for (var i = 0; i < testResults.length; i++) {
      ...
      html += getFeatureWithScenarios(getFeature(testResults[i], scenariosNumberInFeature, passedScenariosNumberInFeature, stepsNumberInFeature, passedStepsInFeature, i + 1) + scenariosHtml);
    }
  ...
  function calculateDuration(duration) {
    ...
    if (elapsedTime >= 60) {
      return parseInt(elapsedTime / 60) + 'm ' + (elapsedTime % 60).toFixed(0).replace(".", ",") + 's';
    } else {
      return elapsedTime.toFixed(2).replace(".", ",") + 's';
    }
  }
